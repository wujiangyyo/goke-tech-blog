...
35B 部署完成后不禁想问：同样的环境，换一个 8B 模型会怎样？于是拉来 **qwen3-vl:8b**（Q4_K_M 5GB + mmproj 1.1GB，6.2GB 全进显存），用**完全相同的测试脚本和素材**跑了一遍对比：

### 速度对比（控制变量实测）

| 指标 | qwen3.6:35b-a3b (IQ3_S) | qwen3-vl:8b (Q4_K_M) | 提升 |
|:---|:---:|:---:|:---:|
| 文本生成 | 33.4 tok/s | **70.9 tok/s** | 2.1x |
| 视觉 decode | 34.6 tok/s | **65.6 tok/s** | 1.9x |
| 文本 prefill | 71 tok/s | **415 tok/s** | 5.8x |
| 图片 prefill | 46 tok/s | 51 tok/s | 1.1x |
| 模型体积 | 14 GB | 6.2 GB | 小一半 |
| 显存占用 | 5.6/8 GB（部分 offload） | ~6 GB（全进显存） | 无内存瓶颈 |

### 能力对比

- **推理题**：8B **5/5 全对**，与 35B 完全平手（CRT/睡莲/骑士骗子/数列/天平全对）
- **视觉识别**：8B 同样准确识别戴胜鸟（学名 Upupa epops、佛法僧目戴胜科），羽冠/虎纹/尾羽细节全对，还给全了学名
- **8B 输出更短更快**：35B 带 thinking 推理链（回复含 `<think>` 过程），8B 直接给答案

### 结论：日常用 8B，35B 当重武器

1. **速度翻倍**：文本 71 vs 33 tok/s，对话响应体感差一倍
2. **推理零损失**：经典推理题测不出差距，都全对
3. **资源更省**：6.2GB 全进显存，无内存带宽瓶颈，功耗更低
4. **适用场景**：8B 适合日常对话/视觉问答/摄像头实时分析；35B 适合复杂推理、长上下文、需要"深度思考"的任务

> 实测打脸了一个预期：原本以为 35B MoE 会明显更聪明，但经典推理题上两者平手。MoE 的"3B 激活"优势主要体现在速度，而 8B 稠密模型全进显存后速度反而翻倍。**本地部署的性价比之王是 8B，不是 35B**。
