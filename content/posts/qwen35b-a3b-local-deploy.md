...
| 真实照片识别（戴胜鸟） | ✅ 准确识别物种及特征（冠羽、黑白条纹翅膀、长喙） |
| 文字生成速度 | **~34 tokens/s**（文本和视觉一致） |
| 图片预填充（prefill） | **~33s**（一张图 ≈ 1500 视觉 token） |

**完整响应时间**：纯文本提问约 4 秒，带图提问约 36 秒——**识图慢在图片预填充阶段，不在文字生成**。视觉编码器把图片切成约 1500 个小块（patch），每个 patch 转成一个 token 一次性灌入，预填充自然比 20 个文字 token 慢一个数量级。

**最惊喜的是真实照片测试**：一张户外的戴胜鸟照片，模型不仅认出是"戴胜鸟（Hoopoe）"，还准确描述了冠羽呈扇形展开、黑白相间的翅膀与尾羽、棕褐色背部、长而弯曲的喙——鸟类辨识级别的细粒度特征全部命中，连石板地面的环境细节也没放过。

### 文字推理能力（5 道经典推理题实测）

为了验证不是"只会聊天"，特意挑了 5 道覆盖不同能力维度的经典推理题，温度 0.3 串行实测：

| 题型 | 题目 | 模型回答 | 结果 |
|:----:|:----|:----|:----:|
| CRT 反直觉 | 球拍+球共 1.10 元，拍贵 1 元，球几钱？ | 0.05 元，设 x 解方程，主动指出"0.1 是陷阱" | ✅ |
| 指数陷阱 | 睡莲 48 天铺满池塘，几天铺一半？ | 第 47 天，逆向推导+科普指数增长误区 | ✅ |
| 逻辑身份 | A 说"我俩都是骗子"，A/B 各是谁？ | A 骗子、B 骑士，反证法三段推理 | ✅ |
| 数列推理 | 2,6,12,20,30,? 下一项 | 42，给出两种规律（二级等差+通项公式）互验 | ✅ |
| 算法思维 | 9 硬币 1 枚重，天平最少称几次？ | 2 次，先用信息论 `3²≥9` 证下界，再给三分法 | ✅ |

**5/5 全对**。每题耗时 10-28 秒（含思考过程）。几个值得注意的表现：

1. **能识别陷阱本身**：CRT 题解出 0.05 后主动说明"多数人会直觉答 0.10"
2. **推理链条完整**：逻辑身份题用反证法（假设 A 是骑士→矛盾→推出 A 是骗子→推出 B 是骑士）
3. **一题多解**：数列题给出两种独立规律互相验证；算法题先用信息论证明最少次数下界，再设计具体分组策略

结论：35B MoE 的推理能力不是"套话式应答"，是真正在推导。作为本地部署模型，这个水平足够日常使用。
...
3. 文字生成速度稳定在 ~34 tokens/s（文本和视觉一致），足够日常对话和编码辅助；但带图提问有约 33 秒的图片预填充开销，适合离线分析而非实时场景
