...
比失忆更致命的是——**Agent 忘了自己写过什么。**

因为技能（Skills）和脚本只存在 Hermes 的技能目录和文件系统中。Agent 在会话中创建或修改了一个 skill，但如果：

- **网关重启** → 新会话加载技能列表，但 Agent 不知道"我刚写了个 xxx skill"
- **上下文压缩** → "刚写的 feed_timem.py 脚本路径是什么"这类信息直接被丢掉
- **跨天使用** → 第二天 Agent 完全不记得昨天创建了什么工具

结果就是：**skill 和脚本明明存在，Agent 却想不起来用它们。** 用户说"用 feed_timem.py 喂数据"，Agent 一脸茫然——它不记得有这个脚本。
...
| 下篇预告 | 语义搜索在 Agent 对话中的实战应用 | 多轮语境感知、记忆优先级排序 |
